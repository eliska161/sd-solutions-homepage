...
export function resolvePdfFontFile(filename: string): string {
  const candidates = [
...
export function pdfFontPaths() {
  return {
    regular: resolvePdfFontFile("LiberationSans-Regular.ttf"),
    bold: resolvePdfFontFile("LiberationSans-Bold.ttf"),
  };
}
...
function drawHeader(doc: PDFKit.PDFDocument, data: JobSummaryDocument) {
  ...
  doc.moveDown(0.25);
  doc.font(fonts.regular).fontSize(10).fillColor(PDF_COLORS.muted);
  ...
  doc.fillColor(PDF_COLORS.accent).text(badgeText, left + 8, badgeY + 4, {
    width: badgeW - 16,
    lineBreak: false,
  });
  doc.y = badgeY + 28;
...
function drawSection(doc: PDFKit.PDFDocument, section: SummarySection) {
  ...
  ensureSpace(doc, 48);
  doc.fillColor(PDF_COLORS.accent).font(fonts.bold).fontSize(11);
  ...
  if (section.bullets?.length) {
    for (const bullet of section.bullets) {
      ensureSpace(doc, 20);
      const y = doc.y;
      doc.fillColor(PDF_COLORS.accent).circle(left + 3, y + 5, 2).fill();
      doc.font(fonts.regular).fontSize(9).fillColor(PDF_COLORS.ink);
