...
const cardClass =
  "rounded-2xl bg-white px-5 py-5 shadow-[0_1px_3px_rgba(16,24,40,0.06),0_8px_24px_rgba(16,24,40,0.04)]";
...
export default async function CustomerStatusPage({
  ...
  const hasServicesBlock =
    data.services.length > 0 || Boolean(data.discount) || data.parts.length > 0;
  const hasUpdates = data.updates.length > 0;
  const hasPhotos = data.photos.length > 0;

  return (
    <main className="mx-auto max-w-md px-5 py-10 sm:py-12 lg:max-w-5xl lg:px-8">
      <header className="mb-8 lg:mb-10">
        <p className="text-[13px] font-semibold tracking-tight text-[#0f766e]">
          SD Solutions
        </p>
        <h1 className="mt-3 text-[28px] font-semibold leading-tight tracking-tight text-zinc-900 lg:text-[32px]">
          Reparasjonsstatus
        </h1>
      </header>

      <div className="grid gap-4 lg:grid-cols-12 lg:gap-5 lg:items-start">
        <section className={`${cardClass} lg:col-span-7`}>
          ...
          <dl className="mt-5 space-y-2.5 border-t border-zinc-100 pt-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:space-y-0 lg:grid-cols-3">
            <div className="flex justify-between gap-4 text-sm sm:flex-col sm:justify-start sm:gap-1">
              <dt className="text-zinc-400">Tekniker</dt>
              <dd className="text-right font-medium text-zinc-800 sm:text-left">
                {data.technicianName}
              </dd>
            </div>
            <div className="flex justify-between gap-4 text-sm sm:flex-col sm:justify-start sm:gap-1">
            ...
            {data.customerPriceLabel ? (
              <div className="flex justify-between gap-4 text-sm sm:flex-col sm:justify-start sm:gap-1">
                ...
                <dd className="text-right font-semibold text-zinc-900 sm:text-left">
                  {data.customerPriceLabel}
                </dd>
              </div>
            ) : null}
        ...
        {hasServicesBlock ? (
          <section
            className={`${cardClass} ${
              hasUpdates ? "lg:col-span-6" : "lg:col-span-12"
            }`}
          >
        ...
        {hasUpdates ? (
          <section
            className={`${cardClass} ${
              hasServicesBlock ? "lg:col-span-6" : "lg:col-span-12"
            }`}
          >
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-zinc-400">
              Oppdateringer
        ...
        {hasPhotos ? (
          <section className={`${cardClass} lg:col-span-12`}>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 lg:gap-3">
              {data.photos.map((p) => (
                <a
                  key={p.id}
                  href={p.url}
      ...
      <footer className="mt-10 text-center text-[11px] text-zinc-400 lg:mt-12">
