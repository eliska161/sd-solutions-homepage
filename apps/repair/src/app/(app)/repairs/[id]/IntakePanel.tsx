...
import { upsertIntakeInspection } from "@/server/intake";
...
export function IntakePanel({
  ...
  function save(markCompleted = false) {
    ...
    startTransition(async () => {
      ...
      setMessage(markCompleted ? "Mottakskontroll lagret og markert utført." : "Lagret.");
      router.refresh();
    });
  }
  ...
  function onUpload(formData: FormData) {
    setMessage(null);
    startTransition(async () => {
      formData.set("file", formData.get("file") as File);
    ...
    <div className="space-y-6">
      {intake?.completedAt ? (
        ...
        <p className="text-[13px] text-muted">
          Registrer fysisk tilstand og funksjon før reparasjonen starter.
        </p>
      )}
      ...
      <div>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          {INTAKE_PHYSICAL_ZONES.map((zone) => (
            <div key={zone.key}>
              <Label htmlFor={`zone-${zone.key}`}>{zone.label}</Label>
              <Input
                id={`zone-${zone.key}`}
                className="mt-1.5"
      ...
      <div>
        <h3 className="text-sm font-medium">Bilder ved innlevering</h3>
        <p className="mt-1 text-[12px] text-muted">
          Ta bilde fra mobilen. Anbefalt: forside, bakside, sider, topp, bunn +
          nærbilder av skader. Interne som standard — gjør synlige for kunde under
          Bilder & filer.
        </p>
        <form
        ...
        >
          <div>
            <Label htmlFor="intake-cat">Vinkel</Label>
            <Select
              id="intake-cat"
          ...
          <div className="flex-1">
            <Label htmlFor="intake-desc">Beskrivelse</Label>
            <Input
              id="intake-desc"
              className="mt-1.5"
          ...
          <div>
            <Label htmlFor="intake-file">Bilde</Label>
            <Input
              id="intake-file"
              name="file"
              type="file"
